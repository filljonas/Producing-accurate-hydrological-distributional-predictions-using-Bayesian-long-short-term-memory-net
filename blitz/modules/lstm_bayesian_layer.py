...
class BayesianLSTM(BayesianRNN):
    ...
    def __init__(self,
        ...
        # TODO: modified
        self.weight_ih_mu = nn.Parameter(torch.zeros(in_features, out_features * 4))
        self.weight_ih_rho = nn.Parameter(torch.Tensor(in_features, out_features * 4).normal_(self.posterior_rho_init, 0.001))
        ...
        self.weight_hh_mu = nn.Parameter(torch.zeros(out_features, out_features * 4))
        self.weight_hh_rho = nn.Parameter(torch.Tensor(out_features, out_features * 4).normal_(self.posterior_rho_init, 0.001))
        ...
        self.bias_mu = nn.Parameter(torch.zeros(out_features * 4))
        self.bias_rho = nn.Parameter(torch.Tensor(out_features * 4).normal_(self.posterior_rho_init, 0.001))
    ...
    def plot_random_weight_dist(self, x_index, y_index):
        """
        ...
        :return:
        """
        mu = self.weight_hh_mu[x_index][y_index].item()
        rho = self.weight_hh_rho[x_index][y_index]
        sigma = torch.log1p(torch.exp(rho)).item()
